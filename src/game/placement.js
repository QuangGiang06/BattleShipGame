...
import { createShip, DEFAULT_FLEET, normalizeOrientation, ORIENTATION } from './ship.js';
...
export function validateFleetPlacements(placements, expectedFleet = DEFAULT_FLEET) {
  if (!Array.isArray(placements)) {
    return { valid: false, reason: 'FLEET_PAYLOAD_INVALID' };
  }

  if (placements.length !== expectedFleet.length) {
    return { valid: false, reason: 'FLEET_SHIP_COUNT_MISMATCH' };
  }

  const ids = placements.map((placement) => placement?.shipId ?? placement?.id ?? placement?.ship?.id);

  if (ids.some((id) => typeof id !== 'string' || !id.trim())) {
    return { valid: false, reason: 'FLEET_SHIP_ID_REQUIRED' };
  }

  if (new Set(ids).size !== ids.length) {
    return { valid: false, reason: 'FLEET_SHIP_ID_DUPLICATED' };
  }

  const actualLengths = placements
    .map((placement) => Number(placement?.ship?.length ?? placement?.length))
    .sort((a, b) => a - b);
  const expectedLengths = expectedFleet.map((ship) => ship.length).sort((a, b) => a - b);

  if (
    actualLengths.some((length) => !Number.isInteger(length) || length <= 0)
    || actualLengths.some((length, index) => length !== expectedLengths[index])
  ) {
    return { valid: false, reason: 'FLEET_COMPOSITION_INVALID' };
  }
